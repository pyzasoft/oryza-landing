...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // Cap device pixel ratio for performance on high-DPI mobiles
    const maxPR = options.pixelRatioCap ?? 2;
    this.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, maxPR));

    this.composer = new EffectComposer(this.renderer);
    container.appendChild(this.renderer.domElement);
    // Ensure canvas fills container on all browsers
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    // Start immediately to avoid mobile browsers deferring until interaction/scroll
    myApp.init();
    // Ensure proper initial sizing on Android where initial layout can be 0x0 until first scroll
    requestAnimationFrame(() => {
      myApp.onWindowResize();
      // Fallback second pass after keyboard/address bar settle
      setTimeout(() => myApp.onWindowResize(), 120);
    });
    // Preload SMAA assets without blocking the start
    myApp.loadAssets();
